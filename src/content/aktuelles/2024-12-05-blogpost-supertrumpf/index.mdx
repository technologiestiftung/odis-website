...
Georeferenzierung meint die Zuweisung von Rauminformationen an einen Datensatz. Ist eine Zuordnung der Werte nach einer Raumeinheit entsprechend der LOR bereits im offenen Datensatz gegeben, kann eine Datenauswertung nach Prognoseraum einfach durch Aggregierung der Werte direkt erfolgen. Eine Georeferenzierung der Daten mit Hilfe der _Prognoseräume.json_ ist dann lediglich für die Darstellung der Datenpunkte in einer Karte nötig.  
Enthält der offene Datensatz Rauminformationen (z.B. Koordinaten) und keine direkte Benennung der Prognoseräume, kann dieser als Geodatensatz bearbeitet werden. Die Python-Bibliothek GeoPandas ist für die Arbeit mit geographischen Daten und die Georeferenzierung sehr nützlich.
Sofern einzelne wenige Einträge leere oder ungültige Geometrien enthalten, besteht die Möglichkeit, diese entweder zu korrigieren oder auszuschließen. Anhand der Überschneidungen der Geometrien beider Datensätze kann schließlich eine Georeferenzierung erfolgen.
...
Bei den Datenauswertungen für den ODIS-Supertrumpf konnte meist EPSG:4326 verwendet werden. Nur in Fällen, in denen es um eine Flächenberechnung geht, wurde auf ein projiziertes Koordinatenreferenzsystem zurückgegriffen.
