...
## Wie steht es um die Metadaten im Berliner Datenportal?

Metadaten müssen vollständig und korrekt vorliegen. Was in unserem Alltag in vielen Bereichen selbstverständlich ist, findet in der Open Data Welt bisher nur bedingt Berücksichtigung. 
...
Dementsprechend haben wir uns die bestehenden Kategorien genauer angeschaut und die Metadatenqualität anhand der folgenden sechs Kategorien untersucht:
-	Dateninhalt
...
Anhand der untenstehenden Datenvisualisierungen kann man sich schnell einen Überblick über die Verteilung der einzelnen Kategorien verschaffen. Demnach besteht insbesondere bei der (doch eigentlich so essentiellen) Beschreibung der Datensätze noch deutlich Luft nach oben.
...
Bei der Menge an Datensätzen war schnell klar, dass eine händische Überprüfung praktisch nicht umzusetzen ist. Inspiriert von Tim Fangmeyer's Projekt [DCATAP_DE AI Analyzer](https://github.com/tifa365/dcatapde_ai_analyzer) haben wir uns daher ebenfalls einem KI-Assistenten bedient. Denn Metadaten sind Informationen in Textform und somit geeignet, ein KI-Sprachmodell zu füttern. Wir haben für diese Auswertung GPT4o-mini von OpenAI verwendet, haben aber auch eine Implementierung mit dem Modell Mistrall Small von der eruopäischen KI-Firma Mistral implementiert. So sind wir Schritt für Schritt vorgegangen:

1. Zuerst haben wir die Metadaten vom [Berliner Datenportal](https://daten.berlin.de) und dessen [CKAN](https://ckan.org/) API heruntergelden. 
...
4. Von den heruntergeladenen Metadaten haben wir nicht alle möglichen Felder dem Modell als Eingabe zur Überprüfung gegeben, sondern lediglich die folgenden: Titel, Beschreibung, geographischer Bezug, geographische Granularität, Tags sowie die zuständige, datenhaltende Stelle.
...
Den Quellcode zu dieser Auswertung haben wir über [GitHub](https://github.com/technologiestiftung/dcatap_ai_analyzer) veröffentlicht.
...
Im nächsten Schritt geben wir den behördlichen Beauftragten für Open Data die vorgefilterten Ergebnisse der Metadatenqualitäts-Bewertung und der KI-Begründung (die auch als Verbesserungsvorschläge funktionieren) der Datensätze im Open Data Portal sowie ein [Handout](https://odis-berlin.de/ressourcen/metadaten_ki/) als Hilfestellung zur Verbesserung der Metadaten mit. So können die Beauftragten "ihre" Metadateneinträge sichten und verbessern. 

Wir sind gespannt, ob sich die Metadatenqualität im Datenportal perspektivisch verbessert. Damit die Suche und Nutzung von Daten die Nerven schont und Zeit bleibt für einen gemütlichen Feierabend mit dem wieder gewonnenen Lieblingsfilm. 
