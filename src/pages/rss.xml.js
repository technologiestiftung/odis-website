...
export async function GET(context) {
  const posts = (await getCollection("projekte"))
    .map(p => ({ ...p.data, link: `/projekte/${p.slug}` }));
  const aktuelles = (await getCollection("aktuelles"))
    .map(p => ({ ...p.data, link: `/aktuelles/${p.slug}` }));
  const events = (await getCollection("veranstaltungen"))
    .map(p => ({ ...p.data, link: `/veranstaltungen/${p.slug}` }));
  const modules = (await getCollection("module"))
    .map(p => ({ ...p.data, link: `/module/${p.slug}` }));
  ...
  let veranstaltungenOverview = await getEntry("veranstaltungenOverview", "veranstaltungenoverview");
  veranstaltungenOverview = { ...veranstaltungenOverview.data, link: "/veranstaltungen" }
  ...
  let aktuellesOverview = await getEntry("aktuellesOverview", "aktuellesoverview");
